...
using DoublyLinkedListAssignment;
...
namespace DoublyLinkedListAssignment.Core
{
    ...
    public class DoublyLinkedList<T> : IEnumerable<T>
    {
        ...
        public void AddLast(T item)
        {
            ...
            {
                tail.Next = newNode; //set current tails next to new node
                newNode.Previous = tail; //set the prev of the new node to the last node
                tail = newNode;
            }
            count++;
        ...
        {
            if (index < 0 || index > count) //validate index range
            {
                throw new ArgumentOutOfRangeException(nameof(index));
            }

            if (index == 0) //handle special cases
            {
                AddFirst(item);
                return;
            }
            ...
            Node<T> curr = GetNodeAt(index); //return node currently at index
            Node<T> newNode = new Node<T>(item); //create a new node
        ...
        {
            Node<T> curr = head; //start from head node
            if (curr == null) //show empty list message if its empty
            {
        ...
        public void DisplayBackward()
        {
            ...
            Node<T> curr = tail; //start from the tail node

            if (curr == null) //empty list message
            {
                Console.WriteLine("The list is empty.");
                return;
            }
            ...
            while (curr != null) //traverse using previous until null
            {
                Console.Write(curr.Data);
        ...
        public T[] ToArray()
        {
            T[] array = new T[count]; //create array of size count

            Node<T> curr = head;
            int i = 0;

            while (curr != null) //traverse and copy elements over
            {
                array[i] = curr.Data;
                curr = curr.Next;
                i++;
        ...
        {
            Node<T> curr = head; //traverse from head to tail

            while (curr != null) //compare each nodes data with the item
            {
        ...
        {
            Node<T> curr = head; //traverse head to tail
        ...
        public T RemoveFirst()
        {
            ...
            {
                throw new InvalidOperationException("The list is empty."); //empty list message
            }

            T removedData = head.Data; //store the data to return

            head = head.Next; //update head

            if (head != null) //if new head is not nut set prev to null
            {
                head.Previous = null;
            }
            else //if list empty, set tail to null
            {
                tail = null;
            }
            ...
            return removedData; //return stored data 
        ...
        public T RemoveLast()
        {
            ...
            T removedData = tail.Data; //store data to return
        ...
        {
            if (index < 0 || index >= count) //valid index range
            {
                throw new ArgumentOutOfRangeException(nameof(index));
            }
            ...
            Node<T> nodeToRemove = GetNodeAt(index); //get node at the index
            T removedData = nodeToRemove.Data; //store data to return
            RemoveNode(nodeToRemove); //remove node
        ...
        public void Reverse()
        {
            if (head == null || head == tail) //check if empty or only has one element
            {
                return;
            }
            ...
            while (curr != null) //traverse list and swap next and previous for each node
            {
                temp = curr.Previous;
                curr.Previous = curr.Next;
                curr.Next = temp;
                curr = curr.Previous; //move to next node (previous before swap)
            }

            if (temp != null) //swaps head and tail
            {
                head = temp.Previous;
                tail = temp.Next;
            }
        ...
        {
            if (index < 0 || index >= count) //validate range
            {
                throw new ArgumentOutOfRangeException(nameof(index));
            }
            Node<T> curr;
        ...
        {
            if (node == head && node == tail) //if only node in list
            {
                head = null;
                tail = null;
            }
