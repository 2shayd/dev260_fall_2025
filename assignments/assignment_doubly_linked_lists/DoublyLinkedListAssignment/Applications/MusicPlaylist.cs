...
namespace DoublyLinkedListAssignment.Applications
{
    ...
    public class MusicPlaylist
    {
        ...
        {
            if (song == null) //validate song is not null
            {
                throw new ArgumentNullException(nameof(song), $"Song's {song} cannot be null.");
            }

            playlist.AddLast(song); //add song to end of playlist using addlast method

            if (currentSong == null) //if first song
            {
                currentSong = playlist.Last; //set as current song
            }
        ...
        {
            if (position < 0 || position > TotalSongs) //valid range
            {
                throw new ArgumentOutOfRangeException(nameof(position), $"Must be between 0 and {TotalSongs}.");
            }

            if (song == null) //validate not null
            {
                throw new ArgumentNullException(nameof(song), "Song cannot be null.");
            }

            playlist.Insert(position, song); //use insert method
        ...
        public bool RemoveSong(Song song)
        {
            ...
            var nodeToRemove = playlist.Find(song); //use find method
            if (nodeToRemove == null)
            {
                return false; //song not found
            }

            if (nodeToRemove == currentSong) //update current song if being removed
            {
                if (currentSong.Next != null)
                {
                    currentSong = currentSong.Next; //move forward
                }
                else if (currentSong.Previous != null)
                {
                    currentSong = currentSong.Previous; //move back
                }
                else
                {
                    currentSong = null; //playlist empty after removal
                }
            }

            bool removed = playlist.Remove(song); //use remove method
        ...
        public bool RemoveSongAt(int position)
        {
            if (position < 0 || position >= TotalSongs) //valid range
            {
                throw new ArgumentOutOfRangeException(nameof(position), $"Must be between 0 and {TotalSongs - 1}.");
            }

            var nodeToRemove = playlist.GetNodeAt(position); //get node using getnodeat method
            if (nodeToRemove == null)
            {
                return false;
            }
            ...
            playlist.RemoveAt(position); //removeat method
        ...
        {
            Console.WriteLine($"Playlist: {Name} ({TotalSongs} songs)"); //show name and song count

            if (!HasSongs) //if its empty message
            {
            ...
            int position = 1;
            foreach (var song in playlist.ToArray())
            {
                string marker = (currentSong != null && currentSong.Data == song) ? "->" : "  ";
                Console.WriteLine($"{marker}{position}. {song.Title} by {song.Artist} ({song.Duration:mm\\:ss})");
                position++;
            }
        ...
        public void DisplayCurrentSong()
        {
            ...
            int position = playlist.IndexOf(song);
            ...
            Console.WriteLine($"Song {position} of {TotalSongs}");
        ...
        {
            return currentSong?.Data; //using null conditional operator if null
