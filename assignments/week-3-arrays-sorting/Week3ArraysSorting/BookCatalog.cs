...

{
    ...
    public class BookCatalog
    {
        ...
        public void StartLookupSession()
        {
            ...
            // Array.ForEach(originalTitles, Console.WriteLine);
            // Array.ForEach(normalizedTitles, Console.WriteLine);
            ...
            while (keepLooking)
            {
                ...
                // TODO: Perform lookup
                PerformLookup(query, originalTitles, startIndex, endIndex);
            }
        ...
        private void SortBooksRecursively()
        {
            ...
            QuickSort(normalizedTitles, 0, bookCount - 1);
            QuickSort(originalTitles, 0, bookCount - 1);
        ...
        private void BuildMultiDimensionalIndex()
        {
            ...
            {
                string normalized = NormalizeTitle(originalTitles[i]);

                char L1 = normalized.Length > 0 && char.IsLetter(normalized[0]) ? normalized[0] : 'A';
                char L2 = normalized.Length > 1 && char.IsLetter(normalized[1]) ? normalized[1] : 'A';

                int r = L1 - 'A';
                int c = L2 - 'A';

                if (startIndex[r, c] == -1) startIndex[r, c] = i;
                endIndex[r, c] = i + 1;
            }
        ...
        private void PerformLookup(string query, string[] orignalTitles, int[,] startIndex, int[,] endIndex)
        {
            // TODO: Normalize query same way as indexing
            string normalizedQuery = NormalizeTitle(query);

            //extract first 1-2 letters
            char L1 = normalizedQuery.Length > 0 && char.IsLetter(normalizedQuery[0]) ? normalizedQuery[0] : 'A';
            char L2 = normalizedQuery.Length > 1 && char.IsLetter(normalizedQuery[1]) ? normalizedQuery[1] : 'A';

            int r = L1 - 'A';
            int c = L2 - 'A';

            //get start and end range from 2D index
            int low = startIndex[r, c];
            int high = endIndex[r, c];

            //show suggestions for nearby ranges
            if (low == -1 || low == high)
            {
                Console.WriteLine($"No matches for '{query}'. Suggestions:");
                //nearby suggestions scan for +/-5 row and column
                int suggestionsCount = 0;
                const int maxSuggestions = 5;
                for (int dr = -1; dr <= 1 && suggestionsCount < maxSuggestions; dr++)
                {
                    for (int dc = -1; dc <= 1 && suggestionsCount < maxSuggestions; dc++)
                    {
                        int nr = r + dr;
                        int nc = c + dc;
                        if (nr >= 0 && nr < 26 && nc >= 0 && nc < 26 && startIndex[nr, nc] != -1)
                        {
                            for (int i = startIndex[nr, nc]; i < endIndex[nr, nc] && i < originalTitles.Length && suggestionsCount < maxSuggestions; i++)
                            {
                ...
                //if nothing nearby to recommend
                if (suggestionsCount < maxSuggestions)
                {
                    for (int i = 0; i < originalTitles.Length && suggestionsCount < maxSuggestions; i++)
                    {
                        if (NormalizeTitle(originalTitles[i]).StartsWith(normalizedQuery[0].ToString()))
                        {
                            Console.WriteLine($"\t{originalTitles[i]}");
                            suggestionsCount++;
                        }
            ...
            int sliceLow = low;
            int sliceHigh = high - 1;

            int index = -1;

            //binary search in slice                
            while (sliceLow <= sliceHigh)
            {
                int mid = (sliceLow + sliceHigh) / 2;
                int cmp = string.Compare(NormalizeTitle(originalTitles[mid]), normalizedQuery, StringComparison.Ordinal);
            ...
            //display results using original titles
            if (index != -1)
            {
                Console.WriteLine($"Found match: {originalTitles[index]} (index {index})");
            }
            else
            {
                Console.WriteLine($"No match found for '{query}'. Nearby suggestions:");
                //recommend 2 before and 2 after
                int insertPos = sliceLow;
                int startSuggestion = Math.Max(low, insertPos - 2);
                int endSuggestions = Math.Min(high, insertPos + 2);

                for (int i = startSuggestion; i < endSuggestions; i++)
                {
                    Console.WriteLine($"\t{originalTitles[i]}");
                }
            }
        ...
        private static int Partition(string[] arr, int low, int high)
        {
            ...
            {
                if (string.Compare(arr[j], pivot, StringComparison.OrdinalIgnoreCase) <= 0)
                {
                    i++;
